...
@app.get("/api/admin/docs/list")
async def get_docs_list():
    """获取文档列表"""
    try:
        if not os.path.exists(DOCS_DIR):
            return {"success": False, "msg": "文档目录不存在"}
...
@app.get("/api/admin/docs/{filename}")
async def get_doc_content(filename: str):
    ...
    try:
        ...
        if not filename.endswith('.md'):
            return {"success": False, "msg": "只支持Markdown文件"}
