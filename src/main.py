...
def parse_datetime(dt_value) -> Optional[datetime]:
    """
    解析多种时间格式，返回带北京时区的 datetime 对象
    
    支持的格式：
    1. 标准 ISO 格式：2026-03-26T15:15:00+08:00
    2. Supabase 格式：2026-03-26 15:15:00 +0800 CST
    3. 带时区和不带时区的情况
    
    返回：带 BEIJING_TZ 时区的 datetime 对象，解析失败返回 None
    """
    if dt_value is None:
        return None
    ...
    try:
        ...
        if isinstance(dt_value, datetime):
            ...
            # 格式1: Supabase 格式 "2026-03-26 15:15:00 +0800 CST"
            if ' CST' in dt_str or ' +0800 ' in dt_str or ' +0000 ' in dt_str:
                # 提取日期和时间部分
                match = re.match(r'(\d{4}-\d{2}-\d{2})\s+(\d{2}:\d{2}:\d{2})', dt_str)
                if match:
                    date_part = match.group(1)
                    time_part = match.group(2)
                    # 检查是否包含 +0800 或 CST
                    if '+0800' in dt_str or 'CST' in dt_str:
                        dt_str_iso = f"{date_part}T{time_part}+08:00"
                    elif '+0000' in dt_str:
                        dt_str_iso = f"{date_part}T{time_part}+00:00"
                    else:
                        dt_str_iso = f"{date_part}T{time_part}+08:00"
                    result = datetime.fromisoformat(dt_str_iso)
                else:
                    # 回退解析
                    dt_str_clean = dt_str.replace(' CST', '').replace(' cst', '').strip()
                    result = datetime.fromisoformat(dt_str_clean.replace(' ', 'T', 1))
            # 格式2: 标准 ISO 格式
            else:
        ...
        logger.warning(f"解析时间失败: {dt_value}, 错误: {e}")
        return None
...
def record_login_failure() -> dict:
    ...
    with _rate_limit_lock:
        ...
        return {
            "locked": False,
            "remaining_attempts": MAX_LOGIN_FAILURES - failure_count,
            "lockout_until": None
        }
...
def set_admin_password(new_password: str) -> bool:
    ...
    try:
        if database_url:
            ...
            # 先检查是否存在
            cursor.execute("SELECT id FROM admin_settings WHERE key = 'admin_password'")
...
def calculate_is_expired(card: dict) -> bool:
    ...
    if card.get('expire_at'):
        expire_at = parse_datetime(card.get('expire_at'))
        if expire_at and expire_at < now:
            return True
    
    # 情况2：激活后N天过期
    if card.get('expire_after_days') and card.get('activated_at'):
        activated_at = parse_datetime(card.get('activated_at'))
        if activated_at:
            # 计算过期时间
...
async def validate_card_key(request: ValidateRequest, fastapi_request: Request):
    ...
    try:
        ...
        # 方式1: 固定日期过期
        if expire_at:
            expire_time = parse_datetime(expire_at)
            if expire_time:
                if now > expire_time:
                    is_expired = True
                    expire_reason = "卡密已过期"
        ...
        # 方式2: 按激活天数过期
            ...
            if activated_time:
                ...
                logger.warning(f"[Validate] 解析激活时间失败: {activated_at}")
...
async def get_card_types(
    ...
    try:
        ...
        # 注意：Supabase 的 order 链式调用顺序是先写的先生效
    ...
    try:
        ...
        for card in cards:
            ...
            if expire_at:
                ...
                if activated_time:
                    expire_time = activated_time + timedelta(days=expire_after_days)
                    actual_expire_at = expire_time.isoformat()
                    if expire_time < now:
                        is_expired = True
        ...
        def matches_filter(item: dict, exclude: str = None) -> bool:
            ...
            if expire_days and expire_days != '' and exclude != 'expire_days':
                ...
                if expire_days == 'expired':
                    ...
                    is_expired = False
                    if item_expire_at:
                        expire_time = parse_datetime(item_expire_at)
                        if expire_time and expire_time < now:
                            is_expired = True
...
async def admin_login(request: LoginRequest, response: Response):
    ...
    if request.password != current_password:
        ...
        return {
            "success": False,
            "msg": f"密码错误，剩余 {result['remaining_attempts']} 次尝试机会",
    ...
    try:
        ...
        for log in logs:
            ...
            if hasattr(access_time_raw, 'strftime'):
                ...
                # 字符串，需要解析
                access_time = parse_datetime(access_time_raw)
                if not access_time:
                    continue  # 解析失败，跳过此条记录
