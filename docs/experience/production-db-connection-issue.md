...
1. 自定义数据库方案（SQLite + PostgreSQL + Supabase 三模式切换）过于复杂
2. 环境变量配置混乱，已导致多次数据丢失
3. 扣子平台已提供完善的开发/生产环境数据库隔离方案
...
- PostgresClient 完整实现
- 复杂的环境判断逻辑
- is_local_dev_mode(), is_production(), get_db_mode()

**修改后**（约 50 行）：
- 只保留 get_db_client() 函数
- 直接调用 PostgresClient
- 移除所有 SQLite 相关代码

#### 2. 增强 postgres_client.py

添加对分离连接参数的支持：
- DATABASE_URL：完整连接字符串
- PGHOST, PGPORT, PGDATABASE, PGUSER, PGPASSWORD：分离参数
...
```toml
[dev]
run = ["sh", "-c", "LOCAL_DEV_MODE=true python -m uvicorn ..."]
```
...
| 项目 | 修改前 | 修改后 |
|------|--------|--------|
| 数据库方案 | SQLite + PostgreSQL + Supabase | PostgreSQL (扣子默认) |
| 环境切换 | 手动配置 LOCAL_DEV_MODE | 平台自动切换 |
| 代码行数 | ~800 行 | ~100 行 |
| 配置复杂度 | 高 | 零配置 |
| 数据安全 | 无保护 | 平台备份+恢复 |
